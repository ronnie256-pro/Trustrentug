...
def client_construction_dashboard(request, app_id=None):
    ...
    if app_id:
        ...
        if project_obj:
            try:
                app = DiasporaClientApplication.objects.get(id=project_obj.id)
            except DiasporaClientApplication.DoesNotExist:
                pass
        else:
    ...
    completed_ids = project_obj.completed_milestones if (project_obj and project_obj.completed_milestones) else []

    total_house_completion = 0.0
    for phase in roadmap_phases:
        earned_phase_weight = 0.0
        for m in phase['milestones']:
            if completed_ids and m['no'] in completed_ids:
                m['status'] = 'completed'
                earned_phase_weight += m['house_pct']
            else:
                m['status'] = 'pending'
    ...
    # Payment Status
    payment_status = {
        'payment_method': app.get_payment_method_display() if app else ''
    }

    # Contract Status
    contract_status = {
        'agreement_no': f"TRUST-AGR-{(app.id if app else project_obj.id):04d}" if (app or project_obj) else "",
        'status': project_obj.get_status_display() if project_obj else ('Approved' if (app and app.status == 'approved') else ''),
        'start_date': project_obj.start_date.strftime('%d %B %Y') if (project_obj and project_obj.start_date) else '',
        'expected_handover': project_obj.expected_completion.strftime('%d %B %Y') if (project_obj and project_obj.expected_completion) else '',
        'legal_representative': 'TRUST Legal & Supervisory Bureau' if (project_obj or app) else ''
    }
